...
**Bracket realism:** the knockout bracket uses the **official 2026 slot template** (from openfootball — e.g. the Round-of-32 positional codes `1A`, `2B`, best-third slots like `3B/E/F/I/J`), resolving each slot from current group standings with a legal assignment of the 8 best third-placed teams. So a team's projected route reflects the real draw, not an Elo-optimized soft path. The only modelled assumption is *who wins* each tie.
